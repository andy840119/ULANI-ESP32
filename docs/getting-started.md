...
3. **接 Tesserae** —— 在「Tesserae」分頁：
  ...
  辨識，不用手動選。
  - **四頁在 Tesserae 上是四台獨立的裝置**，各有自己的 token 和 dashboard，所以每一頁
   要一組自己的配對碼（配對碼是一次性的）。板子在配對時會自己報上
   `ULANI <板子 MAC> page 1`～`4` 這樣的名字，Tesserae 上就看得出哪張卡片對應哪一頁；
   MAC 在名字裡是為了同一台 server 底下接兩片 ESP32 時不會撞名。
