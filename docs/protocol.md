...
- 觀察過 `5` 和 `6` 兩個值，所以**不是 0–5 的格數**（格數不會出現 6）。
- 但也不像單純的百分比：實測**充電充到底也只停在 5**，短暫跳過 6。要嘛這顆電池／
  充電真的有問題，要嘛這個 byte 根本不是百分比。兩種都還沒被排除。
- [ULANI.node.js](https://github.com/Grassboy/ULANI.node.js) 的 `getBatteryLevel()`
  也只是把原始回覆丟出來，沒有做任何解析，所以那邊沒有第二個真相來源可以對。

在確認之前，這個值**照原樣傳遞、不做換算**：後台直接當百分比顯示並把原始回覆
(`0x0605`) 並排印在旁邊，Tesserae 的 heartbeat 也照原樣送 `battery_pct`（見
`components/tesserae/src/tesserae.c` 的 `post_status()`）。這樣假設一旦錯了會立刻
看得出來，而不是安靜地誤導人。**要修正刻度的話，這三個地方要一起改。**
